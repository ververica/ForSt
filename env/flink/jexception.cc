 ...
 */
static char* getExceptionUtilString(JNIEnv *env, jthrowable exc, const char *methodName)
{
    jthrowable jthr;
    jvalue jVal;
    jstring jStr = NULL;
    ...
    jStr = static_cast<jstring>(jVal.l);
...
jthrowable newRuntimeError(JNIEnv *env, const char *fmt, ...)
{
    ...
    return static_cast<jthrowable>((exc) ? exc : out);
}
